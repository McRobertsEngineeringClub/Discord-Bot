...
let encouragements = [...starterEncouragements];
let responding = true;
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('inspire')
        .setDescription('Get an inspiring quote'))
    .addSubcommand(subcommand =>
      subcommand
        .setName('add')
        ...
        .addStringOption(option =>
          option.setName('message')
            .setDescription('The encouraging message to add')
            .setRequired(true)))
    .addSubcommand(subcommand =>
      subcommand
        .setName('delete')
        ...
        .addIntegerOption(option =>
          option.setName('index')
            .setDescription('The index of the message to delete')
            .setRequired(true)))
    .addSubcommand(subcommand =>
      subcommand
        .setName('list')
        .setDescription('List all encouraging messages'))
    .addSubcommand(subcommand =>
      subcommand
        .setName('responding')
        ...
        .addBooleanOption(option =>
          option.setName('state')
            .setDescription('Turn responding on (true) or off (false)')
            .setRequired(true))),
  async execute(interaction) {
    ...
    switch (subcommand) {
      ...
      case 'add':
        ...
        await interaction.reply('New encouraging message added.');
        break;
      case 'delete':
        const index = interaction.options.getInteger('index');
        if (index >= 0 && index < encouragements.length) {
          encouragements.splice(index, 1);
          await interaction.reply('Encouraging message deleted.');
        } else {
          await interaction.reply('Invalid index.');
        }
        break;
      case 'list':
        await interaction.reply(encouragements.join('\n'));
        break;
      case 'responding':
        responding = interaction.options.getBoolean('state');
        await interaction.reply(`Responding is now ${responding ? 'on' : 'off'}.`);
        break;
    }
  },
